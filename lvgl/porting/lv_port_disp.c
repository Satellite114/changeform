...
#include "lcd.h"
#include "main.h"
extern volatile uint16_t Bcolor;
extern volatile uint16_t Fcolor;
/*********************
 *      DEFINES
 *********************/
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    for (y = area->y1; y <= area->y2; y++)
    {
        for (x = area->x1; x <= area->x2; x++)
        {
            ...
            Bcolor = *((uint16_t *)color_p);
            Fcolor = *((uint16_t *)color_p);
            LCD_SetCursor(x, y);
            LCD_WriteRAM_Prepare();
            lcd_wr_data(Fcolor & 0x00ff);
            lcd_wr_data(Fcolor >> 8);
            color_p++;
        }
    }
