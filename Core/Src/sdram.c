...
void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram)
{
	...
	temp=(u32_t)SDRAM_MODEREG_BURST_LENGTH_1          |	//设置突发长度:1(可以是1/2/4/8)
...
//regval:模式寄存器的定义
//返回值:0,正常;1,失败.
uint8_t SDRAM_Send_Cmd(uint8_t bankx,uint8_t cmd,uint8_t refresh,u16_t regval)
{
    u32_t target_bank=0;
...
//WriteAddr:要写入的地址
//n:要写入的字节数
void FMC_SDRAM_WriteBuffer(uint8_t *pBuffer,u32_t WriteAddr,u32_t n)
{
	for(;n!=0;n--)
	{
...
//ReadAddr:要读出的起始地址
//n:要写入的字节数
void FMC_SDRAM_ReadBuffer(uint8_t *pBuffer,u32_t ReadAddr,u32_t n)
{
	for(;n!=0;n--)
	{
