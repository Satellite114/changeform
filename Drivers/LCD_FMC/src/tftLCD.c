...
//    printf("LCD ID:%x\r\n",lcddev.id); //打印LCD ID
    ...
    {
        //重新配置写时序控制寄存器的时序
        FMC_Bank1E_R->BWTR[6]&=~(0XF<<0);//地址建立时间(ADDSET)清零
        FMC_Bank1E_R->BWTR[6]&=~(0XF<<8);//数据保存时间清零
        FMC_Bank1E_R->BWTR[6]|=3<<0;		//地址建立时间(ADDSET)为3个HCLK =18ns
        FMC_Bank1E_R->BWTR[6]|=2<<8; 	//数据保存时间(DATAST)为6ns*3个HCLK=18ns
    }
